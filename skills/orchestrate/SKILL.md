...
> **Layout — this skill is symlinked; it is NOT missing its engine.** `~/.claude/skills/orchestrate/` is a symlink to `~/Code/orchestrate/skills/orchestrate/` and intentionally contains only this `SKILL.md`. The runnable machinery — `recipes/` (`*.example.json` + `local/`), `prompts/agents/` (`*.example.md` + `local/`), `config.json`, `README.md` — lives at the **repo root `~/Code/orchestrate/`**, one level up from `skills/`. `/orchestrate` is **prompt-defined**: the runner is the agent itself reading the recipe JSON + agent prompt templates and dispatching stateless subagents via the Task tool. There is **no separate binary/engine to install**. So seeing "only SKILL.md" under `skills/orchestrate/` does **not** mean orchestrate is unconfigured — follow the symlink to the repo root. `code-writer` / `code-writer-once` run today via the `*.example.json` fallback; `feature-scoper` fast-fails only because its `technical-writer`/`product-lead` agent prompts still carry `[TODO]` personas.
