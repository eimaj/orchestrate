...
Each sub-task in the brief's `## Operations` section must declare a `files` scope list.

Before dispatching any agent:

1. Read all sub-task `files` lists from the brief's `## Operations` section.
...
3. Compose fully self-contained prompt for each: inject only that sub-task's brief slice (goal + files scope), working dir, session_id, run mandate, and resolved skills.
...
2. Apply them to `{artifact_root}/runs/<session_id>/brief.md` sequentially (one at a time, in sub-task order — no concurrent writes).
...
- Input: full brief (all sub-tasks) + all sub-task diffs/outputs merged + session_id.
...
Apply the integration critic's `decision_log_entries` to `brief.md`. Append its learnings.
...
- Run via `_handler_sequential.md` logic: one sub-task at a time, in brief order.
