...
- **Adoption gate — run both checks before dispatch. A brief authored elsewhere is untrusted input.**
  1. **Resolve every `file:line` and code anchor against an explicit ref.** For each anchor the brief cites, run `git grep <symbol> <ref> -- <path>` at the ref the brief names (or `origin/main` if it names none) and confirm it resolves. Anchors read out of a checked-out feature branch are the recurring failure: they render identically to verified ones and a header line asserting "all anchors verified" is not evidence. Rewrite any anchor that does not resolve as `unverified — <what was searched, at which ref>`, or drop it, and record the correction as a Decision Log entry. A phantom `do not change X` constraint is the dangerous case — it can forbid a change the run actually needs.
  2. **Reconcile the brief's build and verification commands against the target repo's own `AGENTS.md` / `CLAUDE.md`.** A brief written for repo X in a *different* repo never had X's agent instructions in context, so it can mandate commands X forbids, or mandate a wrapper chain while omitting the scoped escape hatch X sanctions. Read X's instructions now and fix the brief's verification section before dispatch: name both the mandated chain and any sanctioned scoped alternative, with the condition for reaching for each. Then **smoke-check each named command against the target package** — a command that selects zero tests must not reach the writer as an acceptance criterion. Verify what it does on an empty selection rather than assuming: in `textnow-mono`, `./test.sh -i -t <pkg>` prints `No Tests To Run - Exiting.` and **exits 1** (`test.sh:283-284`), so it fails loudly rather than passing vacuously. There is no scoped *unit* gate to fall back to — `test.sh:88-91` rejects `-t` alongside `-u` ("The use of -t for targets currently only works for integration tests"), and `run_unit_tests()` queries `attr("size", "small|medium", tests(//...))` across the whole repo. Name in the brief which gate actually covers the package, and say so when none does.
